---
title: "Fire-Ants-Mtg02"
author: "Jason Battles"
date: "July 15, 2016"
output: 
  ioslides_presentation: 
    highlight: espresso
---
## Administrivia
- Geek Week - 22AUG (Moran)
- Team Logo (Moran)
- Weekly Scrum (Battles) 
- Expenses (Battles)    
- New News & Learning Updates? (All)

## Investigate Pitch F/X MLB Data (Battles)

here is the [MLB Game Day data](http://gd2.mlb.com/components/game/mlb/)

here is a [sample inning XML file](sample-data/gid_2015_09_05_texmlb_anamlb_1/inning/inning_all.xml)

Pitch F/X Overview from Data Potato
http://www.datapotato.net/research/mlb-data


## HitterValue - Quantitative (I)
Pitch               Value
--------            --------
Hit Sharply-on base 1.5
Hit Sharply-out     1.5
FlyBall-on base hit 1
FlyBall-out         0.8
Grounder-on base hit0.5
Grounder-out        0
Swinging Strike     -0.3
Called Strike       -0.3

## HitterValue - Quantitative (II)
Pitch           Value 
-----           -----
Foul            -0.3
Ball            0
Ball In Dirt    0
In play, run(s) 0
In play, no out 0   
Pitchout        0
Hit By Pitch    0

## HitterValue - Qualitative (I)
![](images/HitterVal-QualitativeScoring.jpg)
                                
## Neo4j Overview (Sweers)

## Wave1 Objectives (Battles)
- Last 25 Outcomes per Batter
- Scatterplot of Pitches per Batter
- Live Game Sonar
- Hit Sonar
- Edge - Node Relationships

## Skills Overview (Sweers)

## Task Tracking (Cohen)

Pivotal Tracker
https://www.pivotaltracker.com/n/projects/1655703

## Next Steps
1. Application Mock-Up
2. Others?

## Summary Notes


                                
